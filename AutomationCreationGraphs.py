...
def main(jsonPath,jsonFile,parsePath,outputPath,repr = "cfg",format = "dot",splitJsonFunc=True):
    ...
    for path_src in tqdm(project):        
        try:
                ...
                # get the start time
                st = time.time()
                s = subprocess.getstatusoutput("joern-parse " + parsePath+ str(folderNum) + ".c")
                if s[0] == 0:
                    print(s[1])
                else:
                    print('Error: {}'.format(s[1]))     
                s2 = subprocess.getstatusoutput("joern-export --repr="+ repr +" --format=" +format+" --out " + outputPath + str(folderNum))
                if s2[0] == 0:
                    print(s2[1])
                    num = num + 1
                else:
    ...
    try:  
      ...
      print('Total Execution time:', time.time()-totalst, 'seconds')
    except:
...
if __name__ == "__main__":
    ...
    jsonFile= "train2.json"
    parsePath= "./dataset/files/"
    outputPath= "./dataset/cpg/"
    repr = "cfg"
    format = "dot"
    splitJsonFunc = True #set to true only if you want to create .c files 
